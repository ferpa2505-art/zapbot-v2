...
class GerenciadorLicenca {
  ...
  _validarChaveLocal(chave) {
    ...
    const planos = { 'BASI': 'basico', 'PROF': 'profissional', 'AGEN': 'agencia', 'DONO': 'vitalicio' }
    const plano  = planos[partes[1]] || 'profissional'
    // Modelo anual: 365 dias por ativação. Plano "vitalicio" (uso próprio/dono) não expira na prática (100 anos).
    const diasValidade = plano === 'vitalicio' ? 100 * 365 : 365
    const expira = new Date(Date.now() + diasValidade * 24 * 3600000).toISOString()
    return { ok: true, plano, expira }
  }

  // ── Recurso pago: IA & Análise (só libera com licença paga ativa, beta não conta) ──
  permiteIA() {
    const v = this.validar()
    return v.valida && v.tipo === 'pago'
  }
  ...
  _gerarChave(plano = 'profissional') {
    const prefixos = { basico: 'BASI', profissional: 'PROF', agencia: 'AGEN', vitalicio: 'DONO' }
